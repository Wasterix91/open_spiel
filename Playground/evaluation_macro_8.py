...
for episode in range(1, NUM_EPISODES + 1):
    ...
    if episode % 1000 == 0:
        # Optional: Zwischenstand mit Winrates
...
if GENERATE_PLOTS:
    ...
    for pid in range(4):
        ...
        counts = [100 * counts_first_per_action[aid][pid] / total_first for aid in range(num_actions)]
        ...
        ax.bar(
            ...
            counts,
            width,
            label=f"Player {pid} ({strat} v{ver}{epi_s}, {winrate:.1f}%)"
        )
    ...
    # === 02 - Gesamte Aktionsverteilung (ohne Pass/Action 0) ===
    non_pass_action_ids = list(range(1, num_actions))
    non_pass_labels = [f"Action {i}" for i in non_pass_action_ids]
    counts_per_action_no_pass = {aid: [action_counts[pid].get(aid, 0) for pid in range(4)]
                                 for aid in non_pass_action_ids}
    ...
    for pid in range(4):
        ...
        counts = [100 * counts_per_action_no_pass[aid][pid] / total_non_pass for aid in non_pass_action_ids]
        strat = PLAYER_CONFIG[pid]["type"]
        ver   = PLAYER_CONFIG[pid].get("version", "")
        epi   = PLAYER_CONFIG[pid].get("episode")
        epi_s = f", ep{int(epi):,}".replace(",", ".") if epi is not None else ""
    ...
    # === 03 - Gesamte Aktionsverteilung (inkl. Pass) — ehemals 01 ===
    ...
    # === 05 - Kombotyp-Anteile je Spieler — ehemals 03 ===
    action_types = {}
    dummy_state = game.new_initial_state()
    for aid in range(num_actions):
        try:
    ...
    # === 06+ - Detaillierte Kombitypenplots — ehemals 04–11 ===
    ...
    for combo, aids in combo_actions.items():
        ...
        # ✅ Warnung vermeiden: obere Grenze mind. 1
