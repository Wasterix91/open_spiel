...
def _pad_episode(ep: int, width: int = 7) -> str:
    """Format episode number like 0430000 to match file names."""
    return f"{ep:0{width}d}"

def _load_ppo_episode(agent, base_dir, version, pid, episode: int):
    """
    Lädt gezielt policy/value state_dicts mit Suffix:
    .../ppo_model_{version}/train/ppo_model_{version}_agent_p{pid}_ep{EEEEEEE}_{policy,value}.pt
    """
    ...
    # Laden auf richtiges Device der Netze
    ...
    # Annahme: agent._policy / agent._value sind nn.Modules
    ...
    # Optional: Merker für spätere Labels
...
def load_agents(player_config, base_dir, game):
    ...
    for pid, cfg in enumerate(player_config):
        ...
        episode = cfg.get("episode")  # optional

        if kind == "ppo":
            # Agent konstruieren (leeres Netz)
            agent = ppo.PPOAgent(
                info_state_size=game.information_state_tensor_shape()[0],
                num_actions=game.num_distinct_actions()
            )

            # Falls Episode angegeben: gezielt policy/value laden
            if episode is not None:
                try:
                    ...
                    # Fallback: altes Restore ohne Episode (wie vorher)
                    model_path = os.path.join(
                        base_dir, f"models/ppo_model_{version}/train/ppo_model_{version}_agent_p{pid}"
                    )
                    agent.restore(model_path)
            else:
                # Kein episode-Feld -> altes Restore
                model_path = os.path.join(
                    base_dir, f"models/ppo_model_{version}/train/ppo_model_{version}_agent_p{pid}"
                )
                agent.restore(model_path)
