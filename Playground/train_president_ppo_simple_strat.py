...
# === 3️⃣ Kartenränge zum Vergleichen
...
# === 4️⃣ Alle Gegner (1, 2, 3) spielen GLEICH: immer die kleinste Karte
def safe_choose(player, state):
    ...
    if not decoded:
        return 0
    # Alle spielen IMMER kleinste Karte
    return min(decoded, key=lambda x: parse_rank(x[1]))[0]

# === 5️⃣ Versionierung: Finde nächste freie Version
...
for ep in progress:
    ...
    while not time_step.last():
        ...
        if steps > 200:
            progress.write(f"⚠️ Episode {ep} aborted after 200 steps")
            break
    ...
    if ep % 100 == 0:
        ...
        progress.write(f"[Ep {ep}] Ø Return: {avg:.2f} | Winrate (last 100): {win_avg:.1f}%")

# === 7️⃣ Agent speichern (train & test Ordner)
...
# Test-Ordner identisch befüllen
import shutil
...
print(f"\n✅ Modell gespeichert unter: {train_dir} & {test_dir}")
...
plt.plot(win_rates, alpha=0.3, label="Win (0/1)")
plt.plot(range(window-1, len(win_rates)), smoothed, label=f"Moving Avg ({window})")
...
print(f"📊 Winrate-Plot gespeichert unter: {plot_path}")
plt.show()
