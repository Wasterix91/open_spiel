# -*- coding: utf-8 -*-
# k1a2_old.py — Alte Version: DQN Single-Agent vs 3 Heuristiken

import os, time, datetime, numpy as np, torch
import pyspiel
from open_spiel.python import rl_environment

from agents.dqn_agent_old import DQNAgent, DQNConfig
...
CONFIG = {
    ...
    # ==== DQN (passt exakt zu deinem alten Agenten) ====
    "DQN": {
        "learning_rate": 3e-4,
        "batch_size": 128,
        "gamma": 0.995,
        ...
        # Pflichtfelder laut altem DQNConfig
        "epsilon_start": 1.0,
        "epsilon_end": 0.05,
        "epsilon_decay": 0.999995,   # alternativ 500_000 → du kannst Feintuning machen
        "soft_target_tau": 0.0,
        "use_double_dqn": True,
    },

    # ======= Rewards (altes System) =======
    "REWARD": {
        "STEP_MODE": "none",
        "DELTA_WEIGHT": 0.5,
...
def main():
    ...
    plotter.log("New Training (k1a2_old): DQN Single-Agent vs Heuristiken")
    ...
    for ep in range(1, CONFIG["EPISODES"] + 1):
        ...
        while not ts.last():
            ...
            if p == 0:
                ...
                agent.store_transition(ob, int(a), float(r_shape), ob_next, bool(ts_next.last()), next_legal)
        ...
        if len(buf) > 0:
            ...
            new_r = float(last.reward + add_env + ep_final_bonus)
            # NamedTuple: (state, action, reward, next_state, done, next_legal_mask)
            buf.buffer[-1] = buf.Experience(
                last.state, last.action, new_r, last.next_state, last.done, last.next_legal_mask
    )
