# -*- coding: utf-8 -*-
# President/training/k4a1_rec.py — PPO (K4 Recommended): Shared Policy + EXTERNAL (4 Sammler, externer Trainer)
# Hinweis: minimale Änderungen gegenüber k4a1_test.py
#  - family-Name geändert (eigener Output-Ordner)
#  - Konfig getuned für robustes Sammeln (mehr Episoden, größere Bundles)
#  - Logik identisch zu k4a1_test.py (Actor–Learner-Trennung via NPZ + Hot-Reload)
...
# ============== CONFIG ==============
CONFIG = {
    "EPISODES":         100_000,    # viele Episoden, Recorder-Style Sammler
    ...
    "EXTERNAL": {
        "ENABLED": False,              # In-Memory-Bundles im selben Prozess (kein Datei-I/O)
        "EPISODES_PER_BUNDLE": 100,    # nutzt denselben Schwellwert für Updates
        "POLL_NEW_POLICY": True,       # bleibt ohne Wirkung, wenn ENABLED=False
        "LATEST_TAG_FILE": "LATEST_POLICY.txt",
    },

    # ======= Rewards =======
    # STEP-Varianten: "none" | "delta_weight_only" | "hand_penalty_coeff_only" | "combined"
    # FINAL-Varianten: "none" | "env_only" | "rank_bonus" | "both"
    # Beibehalten wie in k4a1_test.py, um Änderungen minimal zu halten
    "REWARD": {
...
def _dump_rollouts_npz(agent, path, meta):
    """Robustes, quasi-atomisches Speichern: erst in RAM (BytesIO), dann auf Disk.
    - Schreibt einen .tmp im *gleichen* Ordner und ersetzt ihn atomar.
    - Falls .tmp/replace wider Erwarten fehlschlägt, fällt es auf Direkt-Schreiben zurück.
    """
    ...
    # 1) Alles in RAM serialisieren (vermeidet Teildateien bei Exceptions)
    buf = agent._buffer
    ...
    np.savez_compressed(
        ...
        meta=np.array(meta_bytes, dtype=np.bytes_),
    )
    data = bio.getvalue()

    # 2) RAM → .tmp im selben Ordner schreiben und flush/fsync
    tmp = f"{path}.tmp"
    try:
        ...
        # Fallback: direkt in Zieldatei schreiben
        try:
            ...
            # Letzter Versuch ohne fsync, damit der Lauf nicht komplett stoppt
            with open(path, "wb") as f:
                f.write(data)
            raise e
...
def main():
    ...
    plotter.log("New Training (k4a1_rec): Shared-Policy PPO + External Trainer")
    ...
    current_policy_tag = "init"  # Start-Tag
    bundle_ep_count = 0
    bundle_idx = 0

    # In-Memory Episoden-Zähler für lokales Sammeln → ein Update
    local_ep_counter = 0
    ...
    for ep in range(1, CONFIG["EPISODES"] + 1):
        ...
        if EXT_ENABLED:
            ...
            if bundle_ep_count >= EPISODES_PER_BUNDLE:
                ...
                # Buffer leeren (wichtig: NICHT trainieren!)
                agent._buffer.clear()
                bundle_ep_count = 0
                bundle_idx += 1
                print(f"[k4_rec] wrote rollout bundle: {out_path}")

                # Optional: neue Policy laden
                if POLL_NEW_POLICY:
                    ...
                    if os.path.isfile(tag_file):
                        try:
                            ...
                            if new_tag and new_tag != current_policy_tag:
                                ...
                                print(f"[k4_rec] loaded new policy tag: {new_tag}")
                        except Exception as e:
                            print(f"[k4_rec] WARNING: failed to load new policy: {e}")

            train_metrics = {}
            train_seconds = 0.0
        else:
            # In-Memory Bundles: N Episoden sammeln → 1 Update (kein Datei-I/O)
            local_ep_counter += 1
            if local_ep_counter >= EPISODES_PER_BUNDLE:
    ...
    plotter.log("K4 (Shared-Policy + External, recommended) abgeschlossen.")
