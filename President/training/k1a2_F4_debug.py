...
def main():
    ...
    base_feat_len = expected_feature_len(feat_cfg)
    state_size = base_feat_len + seat_id_dim

    # ---- Start-Logs: Klarheit über Feature-Dimensionen (für USE_HISTORY/SEAT_ONEHOT)
    print("[FEATURES] USE_HISTORY:", CONFIG["FEATURES"]["USE_HISTORY"])
    print("[FEATURES] SEAT_ONEHOT:", CONFIG["FEATURES"]["SEAT_ONEHOT"], "=> seat_id_dim:", seat_id_dim)
    print("[FEATURES] expected_feature_len(feat_cfg):", base_feat_len,
          "=> final observation_dim (state_size):", state_size)

    # ---- Agent, Reward-Shaper ----
    agent = DQNAgent(state_size, A, DQNConfig(**CONFIG["DQN"]))
    # >>> Debug-Dumps der Trainings-Batches aktivieren (A): <<<
    agent.enable_debug(True, max_batches=2)  # z.B. nur die ersten 2 Batches dumpen
    ...
    for ep in range(1, CONFIG["EPISODES"] + 1):
        ...
        ts = env.reset()
        # in k1a2.py, direkt nach: ts = env.reset()
        # nach: ts = env.reset()
        # direkt nach: ts = env.reset()
        """         if ep == 1:
            # 1) ROH aus der Environment (immer 14 bei Deck=16 & 4 Spielern)
            s0_raw = np.array(env._state.observation_tensor(0), dtype=np.float32)
            print(f"[EP{ep}] Startzustand P0 (RAW from env): len={len(s0_raw)}")
            print(s0_raw.tolist())

            # 2) Agenten-Features (nach fit_tensor & optional Seat-One-Hot)
            s0_agent = augment_observation(s0_raw, player_id=0, cfg=feat_cfg)   # schneidet History ggf. ab
            s0_agent = _with_seat_onehot(
                s0_agent, p=0, num_players=num_players, enabled=CONFIG["FEATURES"]["SEAT_ONEHOT"]
            )
            print(f"[EP{ep}] Startzustand P0 (Agent-Input): len={len(s0_agent)} (state_size={state_size})")
            print(s0_agent.tolist())

            # Konsistenzcheck (nur Länge; keine Summen/Normalisierung)
            assert len(s0_agent) == state_size, "len(s0_agent) != state_size"
            exit() """
        ...
        while not ts.last():
            ...
            """    if ep == 1 and ts.observations["current_player"] == 0:
                print(f"[DEBUG] Agent-Input (USE_HISTORY={CONFIG['FEATURES']['USE_HISTORY']}, "
                    f"SEAT_ONEHOT={CONFIG['FEATURES']['SEAT_ONEHOT']}): len={len(s)}\n{s.tolist()}")
                exit() """
