...
mpl.rcParams.update({
    ...
    "legend.fontsize": 14,        # Größe der Legenden-Labels
    "legend.title_fontsize": 18,  # falls du Legend-Titel nutzt
})
...
class MetricsPlotter:
    def __init__(
        ...
        name_prefix: Optional[str] = None,         # <--- NEU
    ):
        self.out_dir = out_dir
        os.makedirs(self.out_dir, exist_ok=True)

        self._out_formats = list(out_formats or ["png"])  # <--- NEU
        self._name_prefix = name_prefix or ""             # <--- NEU
    ...
    def _savefig(self, out_path_no_change: str, *, dpi: int = 300):
        ...
        # k1a2_51_09_rewards_final_components
        if self._name_prefix:
            base_name = f"{self._name_prefix}_{base_name}"

        for fmt in self._out_formats:
            out_path = os.path.join(d, f"{base_name}.{fmt}")
            plt.savefig(out_path, bbox_inches="tight", transparent=False, dpi=(dpi if fmt != "svg" else None))
    ...
    def plot_benchmark(
        ...
        # Achsen/Styling
        def _apply_common_axes(title: Optional[str] = None,
                            add_25: bool = False,
                            label_size: int = 28,   # <<< Achsen-Labels (Text)
                            tick_size: int = 24,    # <<< Achsen-Zahlen
                            title_size: int = 40):  # <<< (falls Titel gesetzt wird)
            if title:  # Titel nur setzen, wenn gewünscht
                plt.title(title, fontsize=title_size)
            plt.xlabel("Episode", fontsize=label_size)
            plt.ylabel("Winrate (%)", fontsize=label_size)
            plt.ylim(0, 100)
            plt.grid(True)
            ax = plt.gca()
            ax.set_xlim(left=0)
        ...
        def _plot_multi(out_path: str, include_macro: bool, add_25: bool = False, suppress_title: bool = False):
            ...
            plt.legend(loc="upper right")  # optional: fontsize=24
            plt.tight_layout()
    ...
    def plot_reward_groups(
        ...
        # <<< NEU: Schriftgrößen nur für diese Rewards-Plots
        font_title: int = 40,
        font_label: int = 28,
        font_tick: int  = 32,
        font_legend: int = 28,
