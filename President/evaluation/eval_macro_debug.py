...
# 1v3 Gegen Max Combo
PLAYER_CONFIG = [
    {"name": "P0", "type": "dqn", "family": "k1a2", "version": "33", "episode": 5000, "from_pid": 0},
...
# --- Quick console dump: Startzustand P0 ---
    state = game.new_initial_state()

    # 1) RAW-Observation aus der Environment (Variante 2 inkl. History)
    obs = np.array(state.observation_tensor(0), dtype=np.float32)
    print(f"[EVAL][RAW obs P0] len={len(obs)}: {obs.tolist()}")

    # 2) Tatsächlicher DQN-Input (auto truncate/pad und ggf. Seat-One-Hot anhängen)
    try:
        in_features = agents[0].q_network.net[0].in_features
    except Exception:
        in_features = obs.shape[0]
    extra = in_features - obs.shape[0]
    if extra <= 0:
        x = obs[:in_features]
    elif extra == NUM_PLAYERS:
        seat = np.zeros(NUM_PLAYERS, dtype=np.float32); seat[0] = 1.0
        x = np.concatenate([obs, seat], axis=0)
    else:
        x = np.concatenate([obs, np.zeros(extra, dtype=np.float32)], axis=0)

    print(f"[EVAL][DQN input P0] len={len(x)}: {x.tolist()}")
    # optional: nur testen und beenden
    # return
    exit()
