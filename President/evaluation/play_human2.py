...
SHOW_HISTORY_IN_OUTPUT = True  # >>> Nur Anzeige: History-Block zusätzlich drucken (Agenten-Input bleibt unverändert)
...
def _dims_for_output():
    """Berechnet deck-/spielerabhängige Basis- und History-Längen für die Ausgabe."""
    num_players = int(GAME_SETTINGS["num_players"])
    num_ranks   = ranks_for_deck(_int_deck_size())
    base_len    = num_ranks + (num_players - 1) + 3
    full_len    = base_len + num_ranks  # +History
    return num_players, num_ranks, base_len, full_len

def print_info_state(state, pid, show_history: bool = None, decimals=None, per_line=None):
    """Nur Ausgabe: zeigt Basis-Features und optional History-Block. Agenten-Input bleibt unberührt."""
    if show_history is None:
        show_history = SHOW_HISTORY_IN_OUTPUT

    vec = np.asarray(state.information_state_tensor(pid), dtype=np.float32)
    L = vec.shape[0]
    NUM_PLAYERS, N_RANKS, BASE_LEN, FULL_LEN = _dims_for_output()

    # Robust: akzeptiere sowohl base- als auch full-Längen
    if L < BASE_LEN:
        print(f"[InfoState P{pid}] (WARN) Unerwartete Länge {L} < erwartetes base {BASE_LEN}")
        print("  " + " ".join(str(int(x)) if float(x).is_integer() else f"{x:.3f}" for x in vec))
        return

    # Slices
    i_hand_end = N_RANKS
    i_opp_end  = i_hand_end + (NUM_PLAYERS - 1)
    i_last_rel = i_opp_end
    i_combo    = i_last_rel + 1
    i_top      = i_last_rel + 2
    i_hist_beg = i_top + 1
    # History-Block ist nur vorhanden, wenn L >= FULL_LEN
    history_present = (L >= FULL_LEN)
    i_hist_end = i_hist_beg + (N_RANKS if history_present else 0)

    # Basis-Felder
    hand_counts = list(map(int, vec[:i_hand_end]))
    opp_counts  = list(map(int, vec[i_hand_end:i_opp_end]))
    try:
        lastp  = int(vec[i_last_rel])
        ntop   = int(vec[i_combo])
        topidx = int(vec[i_top])
    except Exception:
    ...
    hdr_r = " ".join(f"{h:>3}" for h in (f"P{i}" for i in range(1, NUM_PLAYERS)))
    print(f"[InfoState P{pid}]")
    print(f"{hdr_l} | {hdr_r} | {'LastP':>5} {'nTop':>4} {'TopRank':>7}")

    left  = " ".join(f"{v:>{colw}d}" for v in hand_counts)
    right = " ".join(f"{v:>3d}" for v in opp_counts)
    print(f"{left} | {right} | {lastp:>5d} {ntop:>4d} {top_name:>7}")

    # Optionaler History-Block (reine Anzeige)
    if show_history and history_present:
        hist = list(map(int, vec[i_hist_beg:i_hist_end]))
        print("History (ausgespielt pro Rang):")
        print(" ".join(f"{h:>{colw}d}" for h in hist))
    elif show_history and not history_present:
        print("(Hinweis) Kein History-Block im InfoState vorhanden.")
    ...
    while not state.is_terminal():
        ...
        if pid == 0:
            if SHOW_INFOSTATE:
                ...
                print_info_state(state, 0, show_history=SHOW_HISTORY_IN_OUTPUT)
