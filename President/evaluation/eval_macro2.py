...
# Plot-Flags
USE_READABLE_LABELS = True       # True = "Pass", "S 7", "P J", ...
...
def main():
    ...
    # --- Legality-Baseline für ERSTEN Zug (Presence je Bucket) ---
    dummy_state_leg = game.new_initial_state()
    def _bucket_for_action_id_leg(aid: int) -> str | None:
        """Mappt Action-ID auf Combo-Bucket. Pass/Unbekannt -> None."""
        try:
            label = dummy_state_leg.action_to_string(0, aid)
        except Exception:
            return None
        if "Pass" in label:   return None
        if "Single" in label: return "Single"
        if "Pair" in label:   return "Pair"
        if "Triple" in label: return "Triple"
        if "Quad" in label:   return "Quad"
        m = re.search(r"(\d+)-of-a-kind", label)
        if m and int(m.group(1)) >= 5:
            return "N-of-a-kind"
        return None

    LEGAL_BUCKETS = ["Single", "Pair", "Triple", "Quad", "N-of-a-kind"]
    first_legal_presence = {pid: {b: 0 for b in LEGAL_BUCKETS} for pid in range(NUM_PLAYERS)}
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        while not state.is_terminal():
            ...
            if callable(agent):
                action = agent(state)
                if action not in state.legal_actions(pid):
                    raise RuntimeError("illegal action:", action)
            else:
                action = choose_policy_action(agent, state, pid).action

            # --- erster Zug des Startspielers (BEVOR apply_action) ---
            if not first_action_done and pid == start_pid:
                # Legality Presence je Bucket
                legal_set = state.legal_actions(pid)
                present_buckets = set()
                for aid in legal_set:
                    b = _bucket_for_action_id_leg(aid)
                    if b is not None:
                        present_buckets.add(b)
                for b in present_buckets:
                    first_legal_presence[pid][b] += 1

                # Erste tatsächlich gewählte Aktion des Startspielers
                first_action_counts[pid][action] += 1
                first_action_done = True

            # Aktion protokollieren + ausführen
            action_counts[pid][action] += 1
            state.apply_action(action)

        # Episode fertig: Returns/Wins
    ...
    # 01 - First Action Distribution (Startspieler) – Plot + CSV
    ...
    # === CSV: First-Action Combo-Buckets (Counts/Percent) ===
    dummy_state = game.new_initial_state()
    ...
    # --- CSV: First-Action Utilization (nur Startzug, nur Buckets) ---
    # Utilization_b = USED / LEGAL * 100
    df_used = pd.DataFrame.from_dict(
        {f"Player {pid}": {b: first_combo_counts[pid][b] for b in BUCKETS} for pid in range(NUM_PLAYERS)},
        orient="index"
    )[BUCKETS]
    df_used.index.name = "Player"

    df_legal = pd.DataFrame.from_dict(
        {f"Player {pid}": {b: first_legal_presence[pid][b] for b in BUCKETS} for pid in range(NUM_PLAYERS)},
        orient="index"
    )[BUCKETS]
    df_legal.index.name = "Player"

    df_util_percent = (df_used / df_legal) * 100.0
    df_util_percent = df_util_percent.round(1)

    df_util_counts = pd.DataFrame(index=df_used.index)
    for b in BUCKETS:
        df_util_counts[f"{b}__USED"]  = df_used[b].astype(int)
        df_util_counts[f"{b}__LEGAL"] = df_legal[b].astype(int)
    df_util_counts["__TOTAL_STARTS__"] = [int(start_counts[pid]) for pid in range(NUM_PLAYERS)]

    util_percent_path = os.path.join(CSV_DIR, "01_first_action_utilization_percent.csv")
    util_counts_path  = os.path.join(CSV_DIR, "01_first_action_utilization_counts.csv")
    df_util_percent.to_csv(util_percent_path, encoding="utf-8")
    df_util_counts.to_csv(util_counts_path,  encoding="utf-8")
    logger.info("First-action utilization (percent) saved: %s", util_percent_path)
    logger.info("First-action utilization (counts) saved: %s",  util_counts_path)
