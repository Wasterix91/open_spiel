...
# ---------- Deutlich größere Schrift (zentrale Vorgaben) ----------
EVAL_FONT = dict(
    TITLE=40,   # Plot-Titel
    LABEL=28,   # Achsentitel
    TICK=24,    # Tick-Beschriftungen
    LEGEND=24,  # Legendentext
)

# In welchen Formaten speichern?
OUTPUT_FORMATS = ("pdf",)           # nur PDF
# OUTPUT_FORMATS = ("pdf", "png")   # PDF + PNG, falls gewünscht

def savefig_multi(fig, path_no_ext: str):
    """Speichert die Figure in allen OUTPUT_FORMATS – path_no_ext ohne Dateiendung übergeben."""
    for fmt in OUTPUT_FORMATS:
        fig.savefig(f"{path_no_ext}.{fmt}", bbox_inches="tight")


def apply_axes(ax, *, title=None, xlabel=None, ylabel=None):
    """Einheitliche, größere Beschriftungen für Achsen/Titel/Ticks."""
    if title:
        ax.set_title(title, fontsize=EVAL_FONT["TITLE"])
    if xlabel:
        ax.set_xlabel(xlabel, fontsize=EVAL_FONT["LABEL"])
    if ylabel:
        ax.set_ylabel(ylabel, fontsize=EVAL_FONT["LABEL"])
    ax.tick_params(axis="both", which="major", labelsize=EVAL_FONT["TICK"])

def legend_inside(ax, loc="upper right", font_size=None):
    """Legende mit größerer Schriftgröße; Standard ist EVAL_FONT['LEGEND']."""
    fs = EVAL_FONT["LEGEND"] if font_size is None else font_size
    return ax.legend(loc=loc, frameon=True, framealpha=0.9, borderaxespad=0.5, fontsize=fs)
...
def main():
    ...
    row_sums_np = action_table_counts_no_pass.sum(axis=1).replace(0, 1)
    ...
    width = min(0.8 / max(NUM_PLAYERS, 1), 0.22)
    fig, ax = plt.subplots(figsize=(min(28, max(12, len(action_labels_no_pass) * 0.7)), 8))
    for pid in range(NUM_PLAYERS):
        ...
        label_name = cfg.get("name", f"P{pid}")
        ax.bar(x + width * pid, counts, width, label=label_name)
    apply_axes(ax, title="First-Action-Distribution (%)",
               xlabel="erste Aktion des Startspielers",
    ...
    legend_inside(ax)
    fig.tight_layout()
    savefig_multi(fig, os.path.join(PLOT_DIR, "01_first_action_distribution_startplayer"))
    plt.close(fig)
    ...
    # ---------------- 02 (NEU) - First-Action by Bucket (Plot, Percent) ----------------
    buckets = list(ACTIVE_COMBO_ORDER)
    x = np.arange(len(buckets))
    width = min(0.8 / max(NUM_PLAYERS, 1), 0.25)
    fig_width = max(12, len(buckets) * 1.0)
    fig, ax = plt.subplots(figsize=(fig_width, 7))
    for pid in range(NUM_PLAYERS):
        row_label = f"Player {pid}"
        percents = [float(fa_percent_combo.loc[row_label, b]) if b in fa_percent_combo.columns else 0.0
                    for b in buckets]
        cfg = PLAYER_CONFIG[pid]
        label_name = cfg.get("name", f"P{pid}")
        ax.bar(x + width * pid, percents, width, label=label_name)
    apply_axes(ax, title="First-Action-Combo (%)",
               xlabel="Gespielte Kombination",
               ylabel="Relative Häufigkeit")
    ax.set_xticks(x + width * (NUM_PLAYERS - 1) / 2)
    ...
    legend_inside(ax)
    fig.tight_layout()
    savefig_multi(fig, os.path.join(PLOT_DIR, "02_first_action_bucket_percent"))
    plt.close(fig)
    ...
    if aid_to_rank:
        ...
        for pid in range(NUM_PLAYERS):
            ...
            label_name = cfg.get("name", f"P{pid}")
            ax.bar(x + width * pid, percents, width, label=label_name)
        apply_axes(ax, title="First-Action-Rank (%)",
                   xlabel="Ausgespielter Rang",
                   ylabel="Relative Häufigkeit")
        ax.set_xticks(x + width * (NUM_PLAYERS - 1) / 2)
        ...
        legend_inside(ax)
        fig.tight_layout()
        savefig_multi(fig, os.path.join(PLOT_DIR, "04_first_action_by_rank_percent"))
        plt.close(fig)
    ...
    apply_axes(ax, title="05 - Anteil von Pass vs. Spiel-Aktionen",
               xlabel="Spieler", ylabel="Anteil an allen Aktionen (%)")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=15, ha="right")
    ax.set_ylim(0, 100)
    legend_inside(ax)
    fig.tight_layout()
    savefig_multi(fig, os.path.join(PLOT_DIR, "05_pass_vs_play"))
    plt.close(fig)
    ...
    apply_axes(ax, title="06 - Anteil gespielter Kombitypen pro Spieler",
               xlabel="Spieler", ylabel="Anteil an allen Aktionen (%)")
    ...
    legend_inside(ax)
    fig.tight_layout()
    savefig_multi(fig, os.path.join(PLOT_DIR, "06_combo_types_per_player"))
    plt.close(fig)
    ...
    for combo, aids in combo_actions.items():
        ...
        width = 0.2
        fig, ax = plt.subplots(figsize=(max(12, len(labels) * 0.7), 6.2))
        max_height = 0
        for pid in range(NUM_PLAYERS):
        ...
        apply_axes(ax, title=f"{combo_plot_index[combo]} - {combo}-Actions pro Spieler",
                   xlabel=f"{combo}-Actions", ylabel="Relative Häufigkeit (%)")
        ax.set_xticks(x + width * (NUM_PLAYERS-1)/2)
        ...
        legend_inside(ax)
        fig.tight_layout()
        basename = f"{combo_plot_index[combo]}_combo_{combo.lower().replace('-','_')}_detailed"
        savefig_multi(fig, os.path.join(PLOT_DIR, basename))
        plt.close(fig)
