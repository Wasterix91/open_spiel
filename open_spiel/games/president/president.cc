...
const GameType kGameType{/*short_name=*/"president",
                         /*long_name=*/"President Game",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new PresidentGame(params));
}
...
class PresidentObserver : public Observer {
 public:
  PresidentObserver(IIGObservationType iig_obs_type)
  ...
  static void WriteObservingPlayer(const PresidentState& state, int player,
                                   Allocator* allocator) {
  ...
  static void WriteSinglePlayerCard(const PresidentState& state, int player,
                                    Allocator* allocator) {
  ...
  static void WriteAllPlayerCards(const PresidentState& state,
                                  Allocator* allocator) {
  ...
  static void WriteCommunityCard(const PresidentState& state,
                                 Allocator* allocator) {
  ...
  static void WriteBettingSequence(const PresidentState& state,
                                   Allocator* allocator) {
  ...
  static void WritePotContribution(const PresidentState& state,
                                   Allocator* allocator) {
  ...
  void WriteTensor(const State& observed_state, int player,
    ...
    auto& state = open_spiel::down_cast<const PresidentState&>(observed_state);
...
PresidentState::PresidentState(std::shared_ptr<const Game> game, bool action_mapping,
...
int PresidentState::CurrentPlayer() const {
  if (IsTerminal()) {
...
void PresidentState::DoApplyAction(Action move) {
  if (IsChanceNode()) {
...
std::vector<Action> PresidentState::LegalActions() const {
...
std::string PresidentState::ActionToString(Player player, Action move) const {
  return GetGame()->ActionToString(player, move);
}

std::string PresidentState::ToString() const {
  std::string result;
...
bool PresidentState::IsTerminal() const {
  return remaining_players_ == 1 || (round_ == 2 && ReadyForNextRound());
}

std::vector<double> PresidentState::Returns() const {
  if (!IsTerminal()) {
...
std::string PresidentState::InformationStateString(Player player) const {
  const PresidentGame& game = open_spiel::down_cast<const PresidentGame&>(*game_);
  return game.info_state_observer_->StringFrom(*this, player);
}

// Observation is card then contribution of each players to the pot.
std::string PresidentState::ObservationString(Player player) const {
  const PresidentGame& game = open_spiel::down_cast<const PresidentGame&>(*game_);
  return game.default_observer_->StringFrom(*this, player);
}

void PresidentState::InformationStateTensor(Player player,
...
void PresidentState::ObservationTensor(Player player,
                                   absl::Span<float> values) const {
  ContiguousAllocator allocator(values);
  const PresidentGame& game = open_spiel::down_cast<const PresidentGame&>(*game_);
...
std::unique_ptr<State> PresidentState::Clone() const {
  return std::unique_ptr<State>(new PresidentState(*this));
}

std::vector<std::pair<Action, double>> PresidentState::ChanceOutcomes() const {
...
int PresidentState::NextPlayer() const {
  ...
  SpielFatalError("Error in PresidentState::NextPlayer(), should not get here.");
}

int PresidentState::RankHand(Player player) const {
...
void PresidentState::ResolveWinner() {
...
void PresidentState::SequenceAppendMove(int move) {
  if (round_ == 1) {
...
void PresidentState::Ante(Player player, int amount) {
  pot_ += amount;
  ante_[player] += amount;
  money_[player] -= amount;
}

std::vector<int> PresidentState::padded_betting_sequence() const {
...
void PresidentState::SetPrivate(Player player, Action move) {
...
std::unique_ptr<State> PresidentState::ResampleFromInfostate(
...
int PresidentState::NumObservableCards() const {
  return suit_isomorphism_ ? deck_.size() / 2 : deck_.size();
}

int PresidentState::MaxBetsPerRound() const { return 3 * num_players_ - 2; }

void PresidentState::SetPrivateCards(const std::vector<int>& new_private_cards) {
...
PresidentGame::PresidentGame(const GameParameters& params)
  ...
  default_observer_ = std::make_shared<PresidentObserver>(kDefaultObsType);
  info_state_observer_ = std::make_shared<PresidentObserver>(kInfoStateObsType);
}

std::unique_ptr<State> PresidentGame::NewInitialState() const {
  return absl::make_unique<PresidentState>(shared_from_this(),
                                       /*action_mapping=*/action_mapping_,
                                       /*suit_isomorphism=*/suit_isomorphism_);
}

int PresidentGame::MaxChanceOutcomes() const {
  if (suit_isomorphism_) {
...
double PresidentGame::MaxUtility() const {
...
double PresidentGame::MinUtility() const {
  // In poker, the utility is defined as the money a player has at the end of
  // the game minus then money the player had before starting the game.
...
std::shared_ptr<Observer> PresidentGame::MakeObserver(
    absl::optional<IIGObservationType> iig_obs_type,
    const GameParameters& params) const {
  if (params.empty()) {
    return std::make_shared<PresidentObserver>(
...
std::string PresidentGame::ActionToString(Player player, Action action) const {
...
TabularPolicy GetAlwaysFoldPolicy(const Game& game) {
  SPIEL_CHECK_TRUE(
      dynamic_cast<PresidentGame*>(const_cast<Game*>(&game)) != nullptr);
