...
// A generalized version of a President poker, a simple but non-trivial poker game
// described in http://poker.cs.ualberta.ca/publications/UAI05.pdf .
//
// Taken verbatim from the linked paper above: "In President hold'em, the deck
...
class PresidentState : public State {
 public:
  explicit PresidentState(std::shared_ptr<const Game> game,
 ...
 private:
  friend class PresidentObserver;
...
class PresidentGame : public Game {
 public:
  ...
  std::shared_ptr<PresidentObserver> default_observer_;
  std::shared_ptr<PresidentObserver> info_state_observer_;
